...
d3.selectAll('ul')
  .on('mousewheel', function(e) {
    if ($('input:hover').length == 0 && $('.inputarea:hover').length == 0) {
      map.zoomTo(map.getZoom() + event.wheelDeltaY * 0.01, {
        offset: differential,
        animate: false
      })
    };
...
function toggleBubble(e) {
  ...
  map.featuresAt(e.point, {
    ...
    output = output
      ...
      .filter(function(n) {
        return n.indexOf('_case') == -1
      })
      .filter(function(n) {
        return n.indexOf('_stroke') == -1
      })
      .reverse();
    ...
    for (var i = 0; i < output.length; i++) {
      if (features.indexOf(output[i]) == -1) {
        features.push(output[i])
      }
      if (map.getZoom() < 15) {}
    };
    ...
    if (current.quickPick || features.length == 1) {
      showEditView(features[features.length - 1], null, latlon)
    } else(
      ...
      .text(function(d, i) {
        return formatText(features[i])
      })
      .on('click', function(d, i) {
        deactivateLayer(features[i]);
        showEditView(d, i, latlon)
      })
  ...
  if ($('.visible').length >= 1) {
    ...
    d3.selectAll('.editing .inputarea')
      .attr('layer', function(d, i) {
        return selectedGroup.LayerMapping[MappingScheme[i]]
      });
...
function setValue(layer, type, prop, value, convert, instant, constant) {
  ...
  switch (convert) {
    ...
    case 'anchor':
      ...
      if (typeof path === 'object') {
        var index = path['stops'].map(function(e) {
          return e[0]
        }).indexOf(value[0]);

        //if currently trans, splice in a new array entry
        if (index == -1) {
          var futurePosition = path['stops'].filter(function(n) {
            return n[0] < value[0]
          }).length;
          var newAnchor = value;
      ...
      switch (inputarea.classed('font')) {
        ...
        case "capping":
          inputarea
            ...
            .append('option')
            .attr('value', function(d, i) {
              return current.capitalization[i][0]
            })
            .attr('selected', function(d, i) {
              if (fonts[i] == value) {
                return true
            ...
            .text(function(d, i) {
              return current.capitalization[i][1]
            });
          break;

        // all other inputs are numerical and wheelable
        default: 
          ...
          if (inputarea.classed('color')) {
            ...
            inputarea
              ...
              .on('mouseleave', function() {
                ...
                inputarea.select('.input')
                  .text(value);
                setValue(layer, type, prop, value, null, {
                  transition: !1
                });
              });
            ...
            draw = function() {
              ...
              inputarea.select('.input')
                .text(hsl.toString());
              setValue(layer, type, prop, hsl.toString(), null, {
                transition: !1
              });
            };
          } else {
...
function updateSliderStop(slider, data, layer, type, prop) {
  ...
  var stopsWithBubbles =
    ...
    .on('mousedown', function(d) {
      ...
      function mousemove() {
        ...
        if (currentx >= 0 && currentx <= 200 && data.map(function(e) {
            return e[0]
          }).indexOf(Math.round(currentx / 10)) == -1) {

          updateSliderStop(slider, data, layer, type, prop);
          currentstop
            .attr('style', function() {
              return 'left:' + currentx + 'px'
            })
      ...
      function mouseup() {
        ...
        console.log(data.map(function(e) {
          return e[0]
        }).indexOf(Math.round(rounded / 10)));

        if (rounded >= 0 && rounded <= 200 && data.map(function(e) {
            return e[0]
          }).indexOf(Math.round(rounded / 10)) == -1) {
    ...
    .on('mousewheel', function(d) {
      ...
      if (newVal <= max && newVal >= min) {
        setValue(layer, type, prop, [zoom, newVal], null, {
          transition: !1
        });
        input.innerHTML = parseFloat(newVal).toFixed(2);
        updateSliderStop(slider, data, layer, type, prop)
  ...
  var input = data;
  var ymax = d3.max(input.map(function(n) {
    return n[1]
  }));
  ...
  if (output.slice(-1)[0] != 20) {
    output.push({
      'x': 20,
      'y': output.slice(-1)[0]['y']
    })
  };
  var scalex = d3.scale
    .linear()
